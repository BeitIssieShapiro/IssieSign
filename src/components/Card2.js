...
function Card2(props) {
  ...
  let imageSrc = props.imageName
    ? imageLocalCall(props.imageName, props.userContent)
    : undefined;

  let image2 = props.imageName2 ? (
    <img
      className="tileImg img2"
      src={imageLocalCall(props.imageName2, props.userContent)}
      alt="card Placeholder"
    ></img>
  ) : (
    ""
  );
  let cardDouble = isMyIssieSign()
  ...
  if (!props.noLink && !props.selected) {
    if (props.cardType === "file") {
      url =
        ...
        originalCategoryId +
        "/" +
        encodeURIComponent(props.cardName) +
        "/" +
        encodeURIComponent(props.videoName);
    } else {
      url =
        "/video/" +
        encodeURIComponent(props.videoName) +
  ...
  const isShared =
    props.editMode && props.userContent && props.shareCart?.exists(sharedName);
      ...
      buttons: [
        {
          ...
          onClick: () => {
            FileSystem.get()
              .deleteWord(originalCategoryId, props.cardName)
              .then(
                () => {
                ...
                //error
                (e) =>
                  props.alert.error(translate("InfoDeleteFailed") + "\n" + e),
              );
          },
        },
        {
  ...
  const onWordMenu = () => {
    const buttons = props.userContent
      ? [
          {
            caption: translate("EditMenu"),
            icon: <Edit />,
            callback: showWordInfo,
          },
          {
            caption: isShared
              ? translate("RemoveFromShareMenu")
              : translate("AddToShareMenu"),
            icon: <Share />,
            callback: addToShare,
          }, //todo unshare icon
          {
            caption: translate("DeleteMenu"),
            icon: <Delete />,
            callback: deleteWord,
          },
        ]
    ...
    if (props.categoryId === FileSystem.FAVORITES_NAME) {
      buttons.push({
        caption: translate("RemoveFromFavorite"),
        icon: <Favorite />,
        callback: removeFromFavorite,
      });
    } else {
      buttons.push({
        caption: translate("AddToFavorite"),
        icon: <Favorite />,
    ...
    if (props.symLink) {
      buttons.push({
        caption: translate("RemoveFromThisCategory"),
        icon: <PlaylistRemove />,
        callback: removeFromThisCategory,
      });
    } else {
      buttons.push({
        caption: translate("AddToAnotherCategory"),
        icon: <PlaylistAdd />,
        callback: addToAnotherCategory,
      });
    }
  ...
  let translatedName = props.translate
    ? translate(props.cardName)
    : props.cardName;

  if (props.asListItem) {
    return (
      <ISLink url={url} className="word-list-item">
        <div className="word-list-item-img">
          <img src={imageSrc} />
        </div>
  ...
  let innerBody = (
    <div
      className="card"
      style={cardDouble}
      theme={getThemeName(props.themeId)}
    >
      <div className={"header " + props.clipType}></div>
      <div className="main">
        {image2}
        {imageSrc ? (
          <img
            className={"tileImg " + (props.imageName2 ? "img1" : "imgSingle")}
            src={imageSrc}
            alt="card Placeholder"
          ></img>
        ) : null}
      ...
      <div className="footer">
        ...
        {props.editMode &&
          !props.noMoreMenu && ( //props.userContent &&
            <TileButton size={24} onClick={onWordMenu}>
  ...
  let body = <div className="rope-container">{innerBody}</div>;
  return (
    <React.Fragment>
      <ISLink
