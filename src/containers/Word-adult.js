...
import { getBooleanFromString, headerSize, isLandscape, isMobile } from "../utils/Utils";


function ScrollBar({ vertical, length, marginStartEnd, marginToContainer, pos }) {
...
function WordAdults(props) {
        ...
        wordsElements = props.words.map((word) => {
            ...
            return <Card2
                ...
                onClick={(url) => {
                    ...
                    props.pubSub.publish({ command: "set-current-word", categoryId: props.categoryId, title: word.name, isFavorite: word.isFavorite });
                }}
                key={word.id}
                cardName={word.name}
    ...
    const scrollPos = isLandscapeView ?
        ...
        (-props.scroll?.x / scrollDivRef.current?.scrollWidth) * window.innerWidth + 5;

    const selectedWord = selected && Array.isArray(props.words) &&
    ...
    return (
        <div style={{
            flex: 1, display: 'flex', flexDirection: isLandscapeView ? 'row-reverse' : 'column', overflow: "auto"
        }}>
            {isLandscapeView ?
                <div ref={scrollDivRef} scroll-marker="1" style={{
                    display: "flex",
            ...
            <ScrollBar
                ...
                length={(isLandscapeView ? window.innerHeight - headerSizeCalc - 15 : window.innerWidth - 10)}
                marginStartEnd={5} marginToContainer={10}
                pos={scrollPos} />
