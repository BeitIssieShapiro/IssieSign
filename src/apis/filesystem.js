...
export default class FileSystem {
  ...
  loadHiddenFolders() {
    ...
    const defaultHiddenCategories = this.index?.categories.filter(
      (cat) => cat.defaultHide === true,
    );
  ...
  async init(defaultContent, appType, pubSub, showCustomFoldersFirst) {
    ...
    if (!isBrowser()) {
      return new Promise(async (resolve, reject) => {
        if (
          (!isElectron() && !window.resolveLocalFileSystemURL) ||
          FileSystem.getDocDir() === undefined
        ) {
          ...
          const waitForFS = new Promise((resolve, reject) =>
            window.requestFileSystem(
              ...
              (fs) => {
                ...
                console.log(
                  "electron base url",
                  window.electronDocumentBaseURL,
                );
                resolve();
              },
              (err) => reject(err),
          ...
          window.resolveLocalFileSystemURL = (path, success, error, isFile) => {
            if (path === FileSystem.getDocDir()) {
              ...
              if (isFile) {
                window.electronFS?.root.getFile(
                  path,
                  { create: false },
                  success,
                  error,
                ...
                window.electronFS?.root.getDirectory(
                  path,
                  { create: false },
                  success,
                  error,
        ...
        window.resolveLocalFileSystemURL(FileSystem.getDocDir(), (docDir) => {
          docDir.getFile(
            FileSystem.INDEX_FILE,
            ...
            (indexFile) => {
              ...
              reader.onloadend = (evt) => {
                ...
                defaultContent.categories.forEach((defCat) => {
                  const existingCat = this.index.categories.find(
                    (ec) => ec.id === defCat.id,
                  );
                  if (existingCat) {
                    //merge cat
                    Object.entries(defCat).forEach(([key, value]) => {
                      if (key == "words") {
                        existingCat.words = defCat.words.concat(
                          existingCat.words,
                        );
                      } else {
                ...
                // Load favorites:
                const favCat = this.index.categories.find(
                  (cat) => cat.id === FileSystem.FAVORITES_ID,
                );
                if (favCat) {
                  favCat?.words.forEach((favWord) => {
                    const srcCat = this.findCategory(favWord.category);
                    if (srcCat) {
                      const srcWord = srcCat.words.find(
                        (w) => w.name === favWord.name,
                      );
                      if (srcWord) {
  ...
  getCategories() {
    ...
    if (cat) {
      cat = cat.filter(
        (c) => this.hideFolders.find((hf) => hf.name === c.name)?.hide !== true,
      );
    }
    return cat || [];
  }
  ...
  async addRemoveSymbolicLink(categoryId, title, isAdd, addIntoCategoryID) {
    ...
    const attributeFieldName =
      addIntoCategoryID === FileSystem.FAVORITES_NAME ? "favorite" : "symLink";
    trace(
      ...
      isAdd,
      addIntoCategoryID,
      attributeFieldName + "=" + isAdd,
    );

    if (isBrowser()) return;
    const favCategory = this.index.categories.find(
      (c) => c.name === addIntoCategoryID,
    );
    if (!favCategory) {
      throw new Error(
        "Add To another category - category " +
          addIntoCategoryID +
          " not found",
      );
    }
    ...
    if (isAdd) {
      ...
      if (favWord) {
        category = this.index.categories.find(
          (c) => c.name === favWord.category,
        );
      } else {
        trace(
          "word not found in category",
          categoryId,
          title,
          favCategory.words.map((w) => w.id + "-" + w.name),
        );
      }
    }
    ...
    if (isAdd) {
      ...
      if (
        !favCategory.words.find(
          (w) => w.name === word.name && w.category == categoryId,
        )
      ) {
        favCategory.words.push({
          ...word,
          category: categoryId,
          [attributeFieldName]: true,
        });
      }
    } else {
      trace("Remove favorite", categoryId, title);
      favCategory.words = favCategory.words.filter(
        (w) => w.name !== word.name || w.category != category.name,
      );
    }
  ...
  saveCategory(label, themeId, imagePath, originalElem, syncOn, pubSub) {
    ...
    return new Promise(async (resolve, reject) => {
      if (!originalElem) {
        ...
        this.mvFileIntoDir(imagePath, dirEntry, "default.jpg")
          .then(() => {
            // Update index.json
            ...
            let addProps = syncOn
              ? { sync: FileSystem.SYNC_REQUEST, themeId }
              : { themeId };
        ...
        // Editing Category
        let indexCategory = this.index.categories.find(
          (c) => c.name === originalElem.name,
        );
        if (!indexCategory) {
        ...
        if (
          ...
          ((syncOn &&
            (indexCategory.sync === FileSystem.SYNC_REQUEST ||
              indexCategory.sync === FileSystem.IN_SYNC)) ||
            (!syncOn &&
              (indexCategory.sync === FileSystem.SYNC_OFF_REQUEST ||
                !indexCategory.sync)))
        ) {
          // no change:
        ...
        indexCategory.sync = FileSystem.getNewSyncState(
          indexCategory.sync,
          syncOn,
        );
        indexCategory.themeId = themeId;
    ...
    return new Promise(async (resolve, reject) => {
      try {
        if (!originalElem) {
          ...
          const addProps = syncOn
            ? { sync: FileSystem.SYNC_REQUEST }
            : undefined;
          ...
          let indexWord = this.findWord(
            originalElem.category,
            originalElem.name,
          );
          if (!indexWord) {
          ...
          if (
            origImageUrl === imagePath &&
            origVideoUrl === videoPath &&
            label === indexWord.name &&
            ((syncOn &&
              (indexWord.sync === FileSystem.SYNC_REQUEST ||
                indexWord.sync === FileSystem.IN_SYNC)) ||
              (!syncOn &&
                (indexWord.sync === FileSystem.SYNC_OFF_REQUEST ||
                  !indexWord.sync)))
          ) {
            // no change:
  ...
  async deleteWord(categoryName, wordName) {
    ...
    this.index.categories.forEach((cat) => {
      if (
        cat.words.find(
          (word) => word.name == wordName && word.category === categoryName,
        )
      ) {
        // remove from this category:
        cat.words = cat.words.filter(
          (word) => !(word.name == wordName && word.category === categoryName),
        );
      }
    });
  ...
  async deleteCategory(category) {
    ...
    this.index.categories = this.index.categories.filter(
      (c) => c.name !== category,
    );
    return this.saveIndex();
  }
      ...
      window.resolveLocalFileSystemURL(
        ...
        (docDir) => {
          ...
          docDir.getFile(
            ...
            //Success
            (indexFile) =>
              indexFile.createWriter((indexFileWriter) => {
                ...
                var blob = new Blob([JSON.stringify(userContentJson)], {
                  type: "text/plain",
                });
  ...
  static getNewSyncState = (currentState, isOn) => {
    if (isOn) {
      if (
        currentState === FileSystem.IN_SYNC ||
        currentState === FileSystem.SYNC_OFF_REQUEST
      ) {
  ...
  async setSync(entity, isOn, triggerSync) {
    if (entity.words) {
      //category
      const indexCat = this.index.categories.find(
        (c) => c.name === entity.name,
      );
      if (!indexCat.userContent) return;
  ...
  async syncOne(entity, parentEntity, categoriesToUnsync) {
    ...
    if (entity.sync === FileSystem.IN_SYNC) {
      if (
        entity.imageFileIdReplaced ||
        entity.renamed ||
        entity.videoFileIdReplaced
      ) {
    ...
    if (entity.sync === FileSystem.SYNC_REQUEST) {
      if (entity.words) {
        ...
        await this.gdriveUpload(
          this.getFilePath(relPath, true),
          relPath,
          folderId,
          false,
          properties,
        ...
        try {
          if (entity.imageName.length > 0) {
            const response = await this.gdriveUpload(
              this.getFilePath(entity.imageName, true),
              entity.imageName,
              parentEntity.folderId,
              false,
              null,
            );
            entity.imageFileId = response.fileId;
          ...
          const response = await this.gdriveUpload(
            this.getFilePath(entity.videoName, true),
            entity.videoName,
            parentEntity.folderId,
            false,
            null,
  ...
  async gdriveUpload(filePath, relPath, folderId, isAppData, properties) {
    ...
    return new Promise(async (resolve, reject) => {
      let rootFolderId = this.index.rootFolderId,
        rootFolderName = getAppRootFolderName();
      ...
      window.plugins.gdrive.uploadFile(
        filePath,
        relPath,
        folderId,
        rootFolderId,
  ...
  static async gdriveDownload(fileId, targetPath, isAnonymous) {
    ...
    return new Promise((resolve, reject) => {
      window.plugins.gdrive.downloadFile(
        targetPath,
        fileId,
        isAnonymous,
        (response) => {
  ...
  static async gdriveDelete(fileId) {
    return new Promise((resolve, reject) => {
      window.plugins.gdrive.deleteFile(
        fileId,
        (response) => {
          resolve(response);
        },
  ...
  async findRootFolder() {
    return new Promise((resolve, reject) => {
      window.plugins.gdrive.findFolder(
        getAppRootFolderName(),
        "",
        (response) => {
          ...
          if (response.folders.length > 1 && this.index.rootFolderId) {
            ...
            if (
              response.folders.some((f) => f.id === this.index.rootFolderId)
            ) {
  ...
  async reconsile() {
    ...
    for (const f of filesAndFolder) {
      ...
      if (catFiles?.length > 0) {
        ...
        const defFile = catFiles.find(
          (catFile) => catFile.name == "default.jpg",
        );
        if (defFile) {
          // download the file
          ...
          if (!localCat) {
            ...
            await FileSystem.gdriveDownload(
              defFile.id,
              fullPath + catName + "/default.jpg",
              false,
            );
        ...
        for (const catFile of catFiles) {
          ...
          if (!localWord) {
            ...
            if (imageFile) {
              await FileSystem.gdriveDownload(
                imageFile.id,
                fullPath + catName + "/" + imageFile.name,
                false,
              );
